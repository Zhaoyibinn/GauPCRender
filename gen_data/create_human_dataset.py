...
def create_shapenet_dataset(dataset_dir, shape_id, output_dataset_dir):
    ...
    command = f'blenderproc run bproc_script_human.py {obj_path} {output_dir} {train_num} train'
    print(command)
    os.system(command)
    time.sleep(2)

    command = f'blenderproc run bproc_script_human.py {obj_path} {output_dir} {val_num} val'
    print(command)
    os.system(command)
    time.sleep(2)
...
dataset_path = '[Your THuman2.0 path]'
# the content of this path should be '0000' '0001' '0002' ... 
output_path = '../data/human'
...
ids_train = split_dict['human']['train']
ids_eval = split_dict['humane']['eval']
...
for i, shape_id in enumerate(ids_all):
    print(f'------------------------- {i} / {N} --------------------')
    create_shapenet_dataset(dataset_path, shape_id, output_path)
    # break