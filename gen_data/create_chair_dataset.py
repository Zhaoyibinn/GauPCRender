...
def create_shapenet_dataset(dataset_dir, shape_id, output_dataset_dir):
    ...
    command = f'blenderproc run bproc_script_chair.py {obj_path} {output_dir} {train_num} train'
    print(command)
    os.system(command)
    time.sleep(2)

    command = f'blenderproc run bproc_script_chair.py {obj_path} {output_dir} {val_num} val'
    print(command)
    os.system(command)
    time.sleep(2)
...
dataset_path = '[Your ShapeNet path]/03001627'
output_path = '../data/chair'
...
ids_train = split_dict['chair']['train']
ids_eval = split_dict['chair']['eval']
