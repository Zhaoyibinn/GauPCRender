...
def create_shapenet_dataset(dataset_dir, shape_id, output_dataset_dir):
    ...
    command = f'blenderproc run bproc_script_shoe.py {obj_path} {output_dir} {train_num} train'
    print(command)
    os.system(command)
    time.sleep(2)

    command = f'blenderproc run bproc_script_shoe.py {obj_path} {output_dir} {val_num} val'
    print(command)
    os.system(command)
    time.sleep(2)
...
dataset_path = '[Your GSO path]/Shoe_data'
output_path = '../data/shoe'
...
ids_train = split_dict['shoe']['train']
ids_eval = split_dict['shoe']['eval']
